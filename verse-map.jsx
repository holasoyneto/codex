  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (dataRef.current) setSoftErr(String(e.message || e));
  ...
  return (
    <div className={`cx-map-body ${touristOn ? "is-tourist" : ""}`}>
      <div className="cx-map-field-wrap">
        ...
        {/* era recolor wash — tinted by the foot scrub's active polity */}
        <div className="cx-mapx-eraglow" aria-hidden="true" />

        {/* in-surface pulsing orb while a new verse resolves (AI fetch) */}
        {refetching ? (
        ...
        {/* keyboard mirror of the markers — visually hidden, focus-revealed
            (crossref.jsx pattern). Every POI is a REAL button. */}
          ...
          <ul className="cx-mapx-alist">
            ...
            {(data.pointsOfInterest || []).map((p, i) => (
              <li key={i}>
                ...
                }}>{poiGlyph(p.kind)} {p.name}</button>
              </li>
            ))}
          </ul>
        </nav>

        {/* TIME SCRUB — drag across the polity chronology; the map recolors */}
        {Array.isArray(data.polities) && data.polities.length > 0 ? (
          <FootScrub key={`${data.place}:${data.lat}`} polities={data.polities} verseYear={data.verseYear} />
        ) : null}
