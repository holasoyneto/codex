// GENERATED from library.jsx by scripts/build-jsx.mjs — do not edit; edit the .jsx and run `npm run build`.
(function () {
// CODEX — Library panel
...
function Library({ activeBookId, activeChapter, onSelectChapter, activeTranslation, onJumpRef }) {
  ...
  const onInputKey = (e) => {
    ...
    if (e.key === "ArrowDown") {e.preventDefault();setActiveIdx((i) => (i + 1) % flatSuggestions.length);setSuggestOpen(true);} else
    if (e.key === "ArrowUp") {e.preventDefault();setActiveIdx((i) => (i - 1 + flatSuggestions.length) % flatSuggestions.length);setSuggestOpen(true);} else
    if (e.key === "Enter") {
      e.preventDefault();
      const s = flatSuggestions[activeIdx];if (s) s.do();
      setQ("");setSuggestOpen(false);
    } else
    if (e.key === "Escape") {setQ("");setSuggestOpen(false);}
  };
  ...
  const renderBook = (b) => {
    ...
    return (/*#__PURE__*/
      React.createElement("div", { key: b.id, className: "cx-lib-book", ref: isOpen ? openRowRef : null }, /*#__PURE__*/
      ...
      React.createElement("span", { className: "cx-lib-name" }, b.name), /*#__PURE__*/
      React.createElement("span", { className: "cx-lib-meta" },
      isActive ? `${activeChapter} / ${b.chapters}` : b.chapters
      )
      ),
      isOpen ? /*#__PURE__*/
      React.createElement("div", { className: "cx-lib-chs", role: "group", "aria-label": `${b.name} chapters` },
  ...
  return (/*#__PURE__*/
    React.createElement("div", { className: "cx-lib" }, /*#__PURE__*/
    React.createElement("div", { className: "cx-lib-search" }, /*#__PURE__*/
    ...
    q ? /*#__PURE__*/
    React.createElement("button", { className: "cx-lib-clear", onClick: () => setQ(""), "aria-label": "Clear" }, "\xD7") : /*#__PURE__*/

    React.createElement("span", { className: "cx-lib-count", title: "Press / to focus this search" }, "/"),

    suggestOpen && flatSuggestions.length > 0 ? /*#__PURE__*/
    React.createElement("div", { className: "cx-lib-suggest", role: "listbox", "aria-label": "Search suggestions" },
    (() => {
      ...
      return blocks.map(([label, items]) => /*#__PURE__*/
      React.createElement(React.Fragment, { key: label }, /*#__PURE__*/
      React.createElement("div", { className: "cx-lib-suggest-h" }, label),
      ...
      React.createElement("button", {
        ...
        onMouseDown: (e) => {e.preventDefault();s.do();setQ("");setSuggestOpen(false);} }, /*#__PURE__*/

      React.createElement("span", { className: "cx-lib-suggest-lbl" }, s.label), /*#__PURE__*/
      React.createElement("span", { className: "cx-lib-suggest-sub" }, s.kind === "verse" ? _renderSnippet(s.sub) : s.sub)
      )
      )
      )
      );
    })(),
    parsed.kind === "text" && !textHits.length && !textLoading ? /*#__PURE__*/
    React.createElement("div", { className: "cx-lib-suggest-empty" }, "no verses contain \"", q, "\" in your cached corpus") :
    null
    ) :
    null
    ), /*#__PURE__*/

    React.createElement("div", { className: "cx-lib-scroll", ref: scrollRef },
    otShown.length > 0 ? /*#__PURE__*/React.createElement("h3", { className: "cx-lib-h" }, window.t && window.t("lib.ot") || "The Old Testament") : null,
    otShown.map(renderBook),
    ntShown.length > 0 ? /*#__PURE__*/React.createElement("h3", { className: "cx-lib-h" }, window.t && window.t("lib.nt") || "The New Testament") : null,
    ntShown.map(renderBook),
    dcShown.map((g) => /*#__PURE__*/
    React.createElement(React.Fragment, { key: g.canon }, /*#__PURE__*/
    React.createElement("h3", { className: "cx-lib-h" }, window.t && window.t(g.labelKey) || g.fallback),
    g.books.map(renderBook)
    )
    ),
    otShown.length === 0 && ntShown.length === 0 && dcShown.length === 0 ? /*#__PURE__*/
    React.createElement("p", { className: "cx-lib-empty" }, window.t && window.t("lib.empty") || "No book by that name.") :
    null
    )
    ));
