...
// v9 FACE TO FACE — the desk
{
  ...
  action: () => {
    if (window.codexDesk && window.codexDesk.on()) window.codexDesk.focus();else
...
},
{
  ...
  action: () => {
    if (window.codexDesk && window.codexDesk.on()) window.codexDesk.open("library");else
    window.dispatchEvent(new CustomEvent("codex:open-library"));
  }
},
{
  ...
  action: () => {
    if (window.codexDesk && window.codexDesk.on()) window.codexDesk.open("study");else
