...
(function () {
  ...
  function CrossRefPanel({ book, bookId, chapter, verse, translation }) {
    ...
    const onJump = useCallback((targetKey) => {
      ...
      if (hops >= 3) emitDepth("crossref-chain", targetKey, 5);
      else emitDepth("crossref-follow", targetKey, 1);
    }, [thread.length]);
    ...
    return (
      <div className="cx-xref-pane" style={paneStyle}>
        <header style={headerStyle}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            <span style={crumbsStyle}>
              {[hostKey, ...thread].map((k, i, arr) => (
                <span key={i}>
                  {i > 0 ? <span style={{ opacity: 0.5, margin: "0 6px" }}>→</span> : null}
                  <b style={{ color: i === arr.length - 1 ? "var(--cx-accent, #7ee0ff)" : "inherit" }}>
                    {formatRef(k)}
                  </b>
                </span>
              ))}
                      ...
                      <div style={compareWrapStyle}>
                        <div style={compareColsStyle}>
                          <div style={compareColStyle}>
                            ...
                            <div style={compareTextStyle}>{snippetFor(currentKey, translation) || <span style={{ opacity: 0.55 }}>Text not cached.</span>}</div>
                          </div>
                          <div style={compareColStyle}>
                            <div style={compareHeadStyle}>{formatRef(r.ref)}</div>
                            <div style={compareTextStyle}>{snip || <span style={{ opacity: 0.55 }}>Text not cached.</span>}</div>
                          </div>
                        </div>
        ...
        <footer style={footStyle}>
          Click a reference to follow it · ← back to return
        </footer>
      </div>
    );
  }
