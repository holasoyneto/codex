...
(function () {
  ...
  function TimelinePanel(ctx) {
    ...
    const nodes = useMemo(() => {
      ...
      for (const e of vis) {
        if (!run.length || (e.year - run[run.length - 1].year) * pxPerYear < 26) run.push(e);
        else { flush(); run = [e]; }
      }
      flush();
    ...
    if (!events || !view) {
      return (
        <div className="cx-tl2-root">
          <div className="cx-tl2-loading"><span className="cx-tl2-orb" /> RESOLVING CHRONOLOGY…</div>
        </div>
      );
    }
    ...
    return (
      <div className="cx-tl2-root">
        <div className="cx-tl2-head">
          ...
          <span className="cx-tl2-meta">{filtered.length} events · {yearLabel(Math.round(view.center - view.span / 2))} ⟷ {yearLabel(Math.round(view.center + view.span / 2))}</span>
        ...
        <div className={"cx-tl2-river" + (panning ? " is-panning" : "")} ref={riverRef}
          ...
          {strata.map(b => {
            ...
            return (
              <div key={b.era.id} className="cx-tl2-strata"
                   style={{
                ...
                <span className="cx-tl2-strata-lbl" style={{ color: b.era.tint }}>{b.era.label}</span>
              </div>
            );
          })}
            ...
            if (n.kind === "cluster") {
              ...
              return (
                <button key={"c" + i}
                  ...
                  <span className="cx-tl2-cluster-core" style={{ width: size + "px", height: size + "px" }}>+{n.list.length}</span>
                </button>
              );
            }
            const e = n.ev;
          ...
          {selected ? (() => {
            ...
            return (
              <div className="cx-tl2-card" style={{ left: cardX + "px" }}
                   onPointerDown={(e) => e.stopPropagation()}
                   role="region" aria-label={`Event detail: ${selected.title}`}>
                <div className="cx-tl2-card-head">
                  ...
                  <span className="cx-tl2-card-cat">{cat ? cat.glyph : ""} {(cat || { label: selected.category }).label}</span>
                  <button className="cx-tl2-card-x" onClick={() => setSelected(null)} aria-label="Close event detail">✕</button>
                </div>
                ...
                {selected.scripture && selected.scripture.length ? (
                  <div className="cx-tl2-card-refs">
                    {selected.scripture.map((r, j) => {
                      ...
                      if (!p) return null;
                      return (
                        <button key={j} className="cx-tl2-refchip" data-osis={r}
                                onClick={() => gotoRef(r)}
                                title={`Open the reader at ${p.display}`}>
                          ✦ READ {p.display}
                        </button>
                      );
                    })}
                ...
                {selected.people && selected.people.length ? (
                  <div className="cx-tl2-card-meta">PEOPLE · {selected.people.join(", ")}</div>
                ) : null}
                {selected.places && selected.places.length ? (
                  <div className="cx-tl2-card-meta">PLACES · {selected.places.join(", ")}</div>
                ) : null}
        ...
        {/* minimap — the whole river at a glance */}
        <div className="cx-tl2-mini" ref={miniRef} role="slider"
             ...
             onKeyDown={(e) => {
               ...
               if (e.key === "ArrowLeft") { e.preventDefault(); animateTo(view.center - stepY, view.span, 200); }
               else if (e.key === "ArrowRight") { e.preventDefault(); animateTo(view.center + stepY, view.span, 200); }
               else if (e.key === "+" || e.key === "=") { e.preventDefault(); animateTo(view.center, view.span * 0.6, 200); }
               else if (e.key === "-") { e.preventDefault(); animateTo(view.center, view.span * 1.6, 200); }
             }}>
          {strata.map(b => (
            <div key={b.era.id} className="cx-tl2-mini-band"
                 style={{
          ...
          {filtered.map(e => (
            <span key={e.id} className="cx-tl2-mini-dot"
