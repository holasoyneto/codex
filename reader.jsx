...
function CodexReaderX({ surface } = {}) {
  ...
  const go = (bookId, ch, v) => {
    ...
    if (window.codexGoto) window.codexGoto(bookId, ch, v || 1);
    else if (window.codexJumpToRef) window.codexJumpToRef(`${(b && b.name) || bookId} ${ch}${v ? ":" + v : ""}`);
  ...
  const selectVerse = (n) => {
    if (window.codexSelectVerse) window.codexSelectVerse(n);
  ...
  return (
    <div className={`cxr ${surface ? "is-" + surface : ""}`} style={{ "--cxr-fs": fs + "px" }}>
      ...
      <header className="cxr-bar">
        ...
        ><i aria-hidden /><span>{(window.t && window.t("reader.redletter")) || "RED-LETTER"}</span></button>
      ...
      {state.fallback && !state.loading ? (
        <div className="cxr-served" role="note">
          ...
          <button className="cxr-served-keep" onClick={() => { if (window.codexSetPrimary) window.codexSetPrimary(tr); }}
              ...
              return (
                ...
                >
                  <button
                    ...
                    title="Verse menu"
                  >{v.n}</button>
                  <span className="cxr-text">{text}</span>
                </div>
              );
            })}
            {!verses.length ? (
